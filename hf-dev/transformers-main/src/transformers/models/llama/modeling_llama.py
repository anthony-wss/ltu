...
@add_start_docstrings(
    "The Audios LLaMA Model outputting raw hidden-states without any specific head on top.",
    LLAMA_START_DOCSTRING,
)
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.audio_encoder = ATModel()
        self.audio_proj = nn.Sequential(nn.LayerNorm(1280, elementwise_affine=False), nn.Linear(1280, config.hidden_size))
    ...
    def forward(
        ...
        if audio_input != None: # [B, 32, 25, 1280], 32 = layer, 25 = seq_len
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if attention_mask is not None and position_ids is None:
            # TODO: check pos id and att mask
